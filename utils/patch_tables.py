...
TARGETS = {
    '1___MM_자재관리.py': [('inventory', 'item_name'), ('po_receipt_summary', 'po_id'), ('invoice_verifications', 'supplier'), ('blanket_order_releases', 'id'), ('payment_schedule', 'supplier'), ('vmi_agreements', 'supplier_name'), ('vmi_replenishments', 'supplier_name'), ('po_change_log', 'changed_field'), ('moving_avg_price', 'item_name'), ('goods_receipt', 'item_name')],
    '2____SD_판매출하.py': [('customers', 'customer_name'), ('sales_targets', 'item_name')],
    '3___PP_생산계획.py': [('inventory', 'item_name')],
    '5___WM_창고관리.py': [('storage_bins', 'bin_code'), ('inbound_inspection', 'item_name')],
    '9_관리자.py': [('user_permissions', 'page_key'), ('allowed_domains', 'domain')]
}
...
'''
    return block[1:] # remove first newline
...
for fname, tables in TARGETS.items():
    ...
    for table, col in tables:
        ...
        # 찾을 패턴: st.dataframe(어떤 변수 콤마가 없는 등)이 FROM {table} 근처에 있다.
        # 가장 단순하게: FROM {table} 뒷쪽에서 가장 먼저 나오는 st.dataframe(변수명 을 찾음.
        idx = content.find(f"FROM {table}")
        if idx == -1: continue

        sub_str = content[idx:]
        m = re.search(r"^[ \t]*(st\.dataframe\(([A-Za-z0-9_]+)[^\n]*\))", sub_str, flags=re.MULTILINE)
        if m:
            full_match = m.group(1)
            df_var = m.group(2)
            # Find indentation
            line_start = sub_str[:m.start()].rfind('\n') + 1
            indent_str = sub_str[line_start:m.start()]
            if not indent_str.isspace() and indent_str != "": indent_str = "            "
            if indent_str == "": indent_str = "            "

            block = get_edit_block(table, col, indent_str, df_var)
            # Replace
