...
TARGETS = {
    '1___MM_자재관리.py': [('inventory', 'item_name'), ('po_receipt_summary', 'po_id'), ('blanket_order_releases', 'id'), ('goods_receipt', 'item_name')],
    '2____SD_판매출하.py': [('customers', 'customer_name'), ('production_plans', '품목명')],
    '5___WM_창고관리.py': [('quality_inspections', '품목명'), ('pick_wave_lines', '품목'), ('pick_waves', '웨이브번호'), ('putaway_tasks', '품목')],
    '6___TM_운송관리.py': [('logistics', '품목명'), ('shipment_events', '품목명'), ('freight_quotes', '품목명')],
...
for fname, tables in TARGETS.items():
    ...
    for table, col in tables:
        ...
        # 좀더 유연한 검색
        # table 이름이 쿼리에 포함된 이후 나오는 첫 st.dataframe()
        parts = content.split(f"{table}")
        if len(parts) < 2: continue
        
        idx = content.find(f"FROM {table}")
        if idx == -1:
            idx = content.find(f"INTO {table}") # fallback
