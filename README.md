...
사내 공급망 전 영역(구매 → 생산 → 품질 → 창고 → 운송 → 판매)을 하나의 애플리케이션에서 관리하는 **경량화 SCM 솔루션**을 목표로 합니다.  
Streamlit + MySQL 기반입니다. 비용면에서는 서버를 가지고있다면 추가비용없이 사용할수 있을테니 그게 좋겠다고 생각했습니다.
아직은 미완성이라 추가적으로 수정중입니다.
