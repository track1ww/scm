...
with tabs[2]:
    ...
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:14px">
        ...
        <div style="font-size:0.9rem;font-weight:700;color:#1a1a1a">AI HS Code 자동 분류</div>
        <span style="font-size:0.68rem;background:#fdf0e4;color:#c9711c;
                     padding:2px 8px;border-radius:4px;font-weight:600">Claude AI · ★★★ · 3순위</span>
    </div>""", unsafe_allow_html=True)

    hs_ai_c1, hs_ai_c2 = st.columns([3,1])
    with hs_ai_c1:
        hs_item_desc = st.text_area("품목 설명 입력 (재질·용도·기능 등 상세히)",
            placeholder="예: 스테인리스 스틸 재질의 주방용 압력솥. 용량 6L, 안전밸브 내장, 가정용.",
            height=80, key="hs_ai_desc")
        hs_extra = st.text_input("추가 정보 (원산지, 용도, 규격 등 선택)",
            placeholder="예: 중국산, 전기제품 아님, 뚜껑 포함", key="hs_ai_extra")
    with hs_ai_c2:
        st.markdown("<div style='height:28px'></div>", unsafe_allow_html=True)
        hs_classify_btn = st.button("🤖 AI 분류", use_container_width=True,
                                    key="hs_ai_run", type="primary")

    if hs_classify_btn:
        if not hs_item_desc.strip():
            st.error("품목 설명을 입력하세요.")
        else:
            prompt = f"""당신은 관세 전문가입니다. 아래 품목 정보를 바탕으로 HS Code를 분류하세요.

[품목 정보]
설명: {hs_item_desc}
추가정보: {hs_extra or '없음'}

다음 형식으로 답변하세요 (JSON 없이 텍스트로):

## 추천 HS Code
- **1순위**: XXXX.XX.XX — (품목명) — 신뢰도: 높음/중간/낮음
- **2순위**: XXXX.XX.XX — (품목명) — 신뢰도: 높음/중간/낮음
- **3순위**: XXXX.XX.XX — (품목명) — 신뢰도: 높음/중간/낮음
...
## 예상 기본 관세율
(한국 기준 예상 세율, 불확실한 경우 범위로 표기)

⚠️ 본 결과는 참고용이며, 정확한 HS Code는 관세청 또는 관세사를 통해 최종 확인하세요."""

            with st.spinner("AI HS Code 분류 중..."):
                try:
                    ...
                    resp = _req.post("https://api.anthropic.com/v1/messages",
                        ...
                        timeout=30)
                    ai_txt = resp.json()["content"][0]["text"]
                    st.markdown(f"""
                    <div style="background:#fffbeb;border:1px solid #fde68a;border-left:3px solid #F59E0B;
                        ...
                        <div style="font-size:0.68rem;font-weight:700;color:#92400e;letter-spacing:.1em;
                                    text-transform:uppercase;margin-bottom:10px">AI HS Code 분류 결과</div>
                        <div style="font-size:0.84rem;color:#1a1a1a;line-height:1.75;
                                    white-space:pre-wrap">{ai_txt}</div>
                    </div>""", unsafe_allow_html=True)
                except Exception as e:
                    st.error(f"AI 분류 오류: {e}")
...
with tabs["track"]:
    ...
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:14px">
        <div style="width:30px;height:30px;border-radius:7px;
                    background:linear-gradient(135deg,#6366F1,#00C896);
                    display:flex;align-items:center;justify-content:center;font-size:15px">🤖</div>
        <div style="font-size:0.9rem;font-weight:700;color:#1a1a1a">AI 화물 상태 분석</div>
        <span style="font-size:0.68rem;background:#e8f3fd;color:#2383e2;
                     padding:2px 8px;border-radius:4px;font-weight:600">Claude AI · ★★★★★</span>
    </div>""", unsafe_allow_html=True)

    ai_c1, ai_c2 = st.columns([3,1])
    ai_focus = ai_c1.selectbox("분석 포커스",
        ["전체 종합","지연 리스크 분석","ETA 예측","통관 이슈 점검"], key="ai_track_focus")
    run_ai = ai_c2.button("🤖 AI 분석", use_container_width=True,
                          key="ai_track_run", type="primary")

    if run_ai:
        conn=get_db()
        bl_full=conn.execute("SELECT * FROM logistics WHERE id=?",(bl_data['id'],)).fetchone()
        evs=conn.execute("""SELECT event_type,event_date,location,description
            FROM shipment_events WHERE bl_id=? ORDER BY event_date,id""",(bl_data['id'],)).fetchall()
        conn.close()

        ev_txt="\n".join([f"- {e['event_date']} | {e['event_type']} | {e.get('location') or ''} | {e.get('description') or ''}"
                           for e in evs]) if evs else "이벤트 없음"

        focus_desc={
            "전체 종합":"화물 진행 현황, 예상 도착일, 리스크 요소를 종합 분석",
            "지연 리스크 분석":"이벤트 패턴에서 지연 가능성·원인·대응방안 집중 분석",
            "ETA 예측":"이벤트 흐름 기반 최종 도착 예정일 추정 및 신뢰도 평가",
            "통관 이슈 점검":"통관 관련 이벤트 분석, 잠재적 지연 요인, 준비 서류 점검",
        }

        prompt=f"""당신은 SCM·무역 물류 전문가입니다. 아래 화물 정보를 분석해 실무적인 한국어 보고서를 작성하세요.

분석 포커스: {focus_desc.get(ai_focus,'')}

[화물 정보]
B/L: {bl_data.get('bl_number','-')} | 운송: {bl_data.get('transport_type','-')} | 운송사: {bl_data.get('carrier','-')} | 상태: {bl_data.get('status','-')}
출발: {bl_full.get('departure_date','-') if bl_full else '-'} | 도착예정: {bl_full.get('arrival_date','-') if bl_full else '-'}
...
## 위험 등급: [낮음/보통/높음] — (이유 1문장)"""
        ...
        with st.spinner("AI 분석 중..."):
            try:
                ...
                st.markdown(f"""
                <div style="background:#f7f7f5;border:1px solid #e9e9e7;border-left:3px solid #6366F1;
                            border-radius:8px;padding:16px 20px;margin-top:4px">
                    <div style="font-size:0.68rem;font-weight:700;color:#6366F1;letter-spacing:.1em;
                                text-transform:uppercase;margin-bottom:10px">AI 분석 · {ai_focus}</div>
                    <div style="font-size:0.84rem;color:#1a1a1a;line-height:1.75;
                                white-space:pre-wrap">{ai_text}</div>
                </div>""", unsafe_allow_html=True)
            except Exception as e:
